...
import { selectSongForPreview, addSongToSchedule } from '../../actions'
    ...
    <div class="${sectionClass}">
      <div class="${headerClass}">
        <div class="${headerLeftClass}">
          ...
          <span class="font-semibold uppercase tracking-[0.5px] text-text-secondary">SONG LIBRARY</span>
    ...
    section.querySelectorAll('.add-schedule-btn').forEach(btn => {
        btn.addEventListener('click', async (e) => {
            ...
            if (song.variations && song.variations.length > 1) {
                ...
                addSongToSchedule(songId, variationId)
            }
        })
    })
...
function showVariationDataPopover(trigger: HTMLElement, song: Song): void {
    ...
    song.variations.forEach(variation => {
        ...
        option.addEventListener('click', () => {
            addSongToSchedule(song.id, variation.id)
            overlay.remove()
        })
